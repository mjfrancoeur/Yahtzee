...
public class Yahtzee extends GraphicsProgram implements YahtzeeConstants {
	...
	public void run() {
		 ...
		 * to keep track of which categories a player has selected.
		 * A player cannot select the same category multiple times
		 */
	...
	/* checks to see if category has been selected in prior turn, if it has prompts player to select a different category */
		while(true) {
	...
	private void tallyTotalScores() {
		...
		for(int i = 0; i < nPlayers; i++) {
			totalPlayerScore = 0;
			totalPlayerScore += scoresArray[i][UPPER_SCORE - 1] + scoresArray[i][UPPER_BONUS - 1] + scoresArray[i][LOWER_SCORE - 1];
	...
	private void displayMessage(int x) {
		...
		switch(x) {
		case 0: display.printMessage(currentPlayerName + "'s turn! Click \"Roll Dice\" button to roll the dice.");
			break;
		case 1:
		case 2: display.printMessage("Select the dice you wish to re-roll and click \"Roll Again\"");
			break;
		case 3: display.printMessage("Select a category for this roll");
			break;
		case 4: display.printMessage("Congratulations " + winningPlayer + ", you're the winner with a total of " + winningScore + "!");
			break;
		}
